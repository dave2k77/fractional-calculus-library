...
class BoundedAlphaParameter(nn.Parameter):
    ...
    def __new__(cls, alpha_init: float = 0.5, alpha_min: float = 0.01, alpha_max: float = 1.99):
        ...
        return super().__new__(cls, rho_init)
    
    def __init__(self, alpha_init: float = 0.5, alpha_min: float = 0.01, alpha_max: float = 1.99):
        # Don't call super().__init__ here as the tensor is already created in __new__
...
def debug_learnable_alpha():
    ...
    print("Debugging Learnable α (Fixed)...")
