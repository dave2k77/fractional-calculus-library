#!/usr/bin/env python3
"""
Simple EEG Classification Experiments for hpfracc
...
class SimpleEEGExperimentRunner:
    """Run simple EEG classification experiments and collect real results"""
    ...
    def run_experiments(self, X, y, epochs_data):
        ...
        print("\n" + "="*60)
        print("SIMPLE EEG CLASSIFICATION EXPERIMENTS")
        ...
        # Train Simple CNN
        cnn_model = SimpleCNN(n_channels, n_times, num_classes).to(self.device)
        ...
        print(f"Simple CNN Accuracy: {cnn_accuracy:.4f} ({cnn_time:.2f}s)")
        results['Simple CNN'] = {'accuracy': cnn_accuracy, 'time': cnn_time}
        ...
        # Compare Random Forest vs Simple CNN
        rf_acc = results['Random Forest']['accuracy']
        cnn_acc = results['Simple CNN']['accuracy']
        
        print(f"Random Forest Accuracy: {rf_acc:.4f}")
        print(f"Simple CNN Accuracy: {cnn_acc:.4f}")
        print(f"Difference: {cnn_acc - rf_acc:.4f} ({((cnn_acc - rf_acc) / rf_acc * 100):.2f}%)")
    ...
    def save_results(self, results, epochs_data):
        ...
        with open('eeg_results/simple_eeg_results.txt', 'w') as f:
            f.write("Simple EEG Classification Results\n")
        ...
        print("Results saved to eeg_results/simple_eeg_results.txt")
        print("Visualization saved to eeg_results/simple_eeg_results_plot.png")
    ...
    def create_visualization(self, results):
        ...
        # Accuracy plot
        bars1 = ax1.bar(methods, accuracies, color=['skyblue', 'lightgreen', 'lightcoral'])
        ax1.set_title('Simple EEG Classification Accuracy Comparison\nASUS TUF A15 (RTX 3050, 30GB RAM)', fontsize=14)
        ...
        # Time plot
        bars2 = ax2.bar(methods, times, color=['skyblue', 'lightgreen', 'lightcoral'])
        ...
        plt.tight_layout()
        plt.savefig('eeg_results/simple_eeg_results_plot.png', dpi=300, bbox_inches='tight')
        plt.close()

def main():
    """Main function to run simple EEG experiments"""
    print("Starting Simple EEG Classification Experiments...")
    print("Phase 1: Real Data Collection on ASUS TUF A15")
    print("Goal: Replace synthetic 91.5% vs 87.6% with real results")
    
    # Initialize data loader
    data_loader = SimpleEEGDataLoader()
    ...
    runner = SimpleEEGExperimentRunner()
