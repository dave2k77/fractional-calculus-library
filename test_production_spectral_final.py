...
class FinalProductionSpectralDerivative(torch.autograd.Function):
    """Final production spectral derivative addressing all review points."""
...
class FinalProductionSpectralLayer(nn.Module):
    """Final production spectral layer with learnable α."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return FinalProductionSpectralDerivative.apply(x, alpha, self.dx, self.mode)


def test_limit_behavior_final():
    """Test α→0 and α→2 limit behavior with improved implementation."""
    print("Testing Limit Behavior (Final)...")
    ...
    result_0 = FinalProductionSpectralDerivative.apply(f, 0.01, dx, "riesz")
    ...
    result_2 = FinalProductionSpectralDerivative.apply(f_smooth, 1.99, dx, "riesz")
...
def test_semigroup_property():
    ...
    # Compute D^α D^β f
    Dbeta_f = FinalProductionSpectralDerivative.apply(f, beta, dx, "riesz")
    Dalpha_Dbeta_f = FinalProductionSpectralDerivative.apply(Dbeta_f, alpha, dx, "riesz")
    
    # Compute D^(α+β) f
    Dalpha_plus_beta_f = FinalProductionSpectralDerivative.apply(f, alpha + beta, dx, "riesz")
...
def test_adjoint_property():
    ...
    Df = FinalProductionSpectralDerivative.apply(f, alpha, dx, "riesz")
    Dg = FinalProductionSpectralDerivative.apply(g, alpha, dx, "riesz")
...
def test_learnable_alpha_final():
    """Test learnable α with final fixed bounded parameterization."""
    print("Testing Learnable α (Final)...")
    ...
    layer = FinalProductionSpectralLayer(alpha_init=0.3, mode="riesz", dx=dx)
...
def test_numerical_stability():
    ...
    try:
        result = FinalProductionSpectralDerivative.apply(f, 0.001, dx, "riesz")
    ...
    try:
        result = FinalProductionSpectralDerivative.apply(f, 1.999, dx, "riesz")
...
def test_dc_mode_handling():
    ...
    result = FinalProductionSpectralDerivative.apply(f, 0.5, dx, "riesz")
...
def test_weyl_vs_riesz():
    ...
    riesz_result = FinalProductionSpectralDerivative.apply(f, alpha, dx, "riesz")
    weyl_result = FinalProductionSpectralDerivative.apply(f, alpha, dx, "weyl")
...
def test_complex_step_derivative_fixed():
    """Test complex-step derivative for α gradients validation (fixed)."""
    print("Testing Complex-Step Derivative for α Gradients (Fixed)...")
...
def test_3_2_dealiasing():
    ...
    result_no_pad = FinalProductionSpectralDerivative.apply(f, alpha, dx, "riesz")
    
    # Test with 3/2 de-aliasing (simplified)
    f_padded = torch.cat([f, f, f], dim=-1)  # Simple padding
    result_padded = FinalProductionSpectralDerivative.apply(f_padded, alpha, dx, "riesz")
...
def test_mixed_precision():
    """Test mixed precision (FP16/BF16) compatibility."""
    ...
    if torch.cuda.is_available():
        try:
            f_fp16 = f.half()
            result_fp16 = FinalProductionSpectralDerivative.apply(f_fp16, alpha, dx, "riesz")
...
def main():
    ...
    print("=" * 60)
    print("FINAL PRODUCTION SPECTRAL AUTOGRAD TEST SUITE")
    print("=" * 60)
    
    tests = [
        ("Limit Behavior (Final)", test_limit_behavior_final),
        ("Semigroup Property", test_semigroup_property),
        ("Adjoint Property", test_adjoint_property),
        ("Learnable α (Final)", test_learnable_alpha_final),
        ("Numerical Stability", test_numerical_stability),
        ("DC Mode Handling", test_dc_mode_handling),
        ("Weyl vs Riesz", test_weyl_vs_riesz),
        ("Complex-Step Derivative (Fixed)", test_complex_step_derivative_fixed),
