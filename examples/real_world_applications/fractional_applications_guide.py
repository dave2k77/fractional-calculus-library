...
def application_4_biomedical_signal_processing():
    ...
    signal = (np.sin(2*np.pi*5*t) + 0.5*np.sin(2*np.pi*10*t) + 
              0.3*np.sin(2*np.pi*15*t) + 0.1*np.random.randn(len(t)))
    ...
    # Plot PSD
    plt.semilogy(freqs[:len(freqs)//2], np.abs(fft_original[:len(freqs)//2])**2, 
                'b-', label='Original signal', linewidth=1.5)
    plt.semilogy(freqs[:len(freqs)//2], np.abs(fft_derivative[:len(freqs)//2])**2, 
                'r-', label='Fractional derivative', linewidth=1.5)
...
def application_5_control_systems():
    ...
    for i in range(1, len(t)):
        ...
        # Solve for one step
        dt = t[1] - t[0]
        y_step = adomian.solve(rhs, np.array([t[i-1], t[i]]), 
                              initial_condition=y[i-1], terms=5)
        y[i] = y_step[-1]
...
def application_7_signal_processing():
    ...
    signal = (np.sin(2*t) + 0.5*np.sin(10*t) + 
              0.3*np.sin(20*t) + 0.1*np.random.randn(len(t)))
    ...
    plt.semilogy(freqs[:len(freqs)//2], np.abs(fft_original[:len(freqs)//2]), 
                'b-', label='Original', linewidth=1.5)
    plt.semilogy(freqs[:len(freqs)//2], np.abs(fft_noisy[:len(freqs)//2]), 
                'r-', label='Noisy', linewidth=1, alpha=0.7)
    plt.semilogy(freqs[:len(freqs)//2], np.abs(fft_filtered[:len(freqs)//2]), 
                'g-', label='Filtered', linewidth=1.5)
