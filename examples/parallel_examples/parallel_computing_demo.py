...
def parallel_backend_comparison():
    ...
    for backend in backends:
        ...
        for N in grid_sizes:
            ...
            results_parallel = parallel_backend.parallel_map(compute_derivative, work_items)
            end_time = time.time()
    ...
    for i, (backend, backend_results) in enumerate(results.items()):
        ...
        plt.loglog(grid_sizes_available, times, 'o-', color=colors[i], 
                  label=backend, linewidth=2, markersize=8)
...
def joblib_optimization_demo():
    ...
    plt.plot(workers, times, 'bo-', linewidth=2, markersize=8, label='Actual Performance')
    ...
    plt.tight_layout()
    plt.savefig('examples/parallel_examples/joblib_scaling.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def vectorized_parallel_demo():
    ...
    def compute_with_alpha(f_data, t_data, alpha_data, h_data):
        caputo = CaputoDerivative(alpha=alpha_data)
        return caputo.compute(f_data, t_data, h_data)
    
    results = parallel_computing.parallel_vectorized_derivative(
        compute_with_alpha,
        f, t, alphas, h
    )
    end_time = time.time()
    ...
    plt.tight_layout()
    plt.savefig('examples/parallel_examples/vectorized_parallel.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def load_balancing_demo():
    ...
    from src.optimisation.parallel_computing import LoadBalancer
    ...
    for strategy in strategies:
        ...
        load_balancer = LoadBalancer(num_workers=num_workers, strategy=strategy)
    ...
    for i, (strategy, result) in enumerate(results.items()):
        ...
        plt.title(f'{strategy.capitalize()} Load Balancing\nImbalance: {result["load_imbalance"]}')
        plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('examples/parallel_examples/load_balancing.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def memory_optimization_demo():
    ...
    plt.tight_layout()
    plt.savefig('examples/parallel_examples/memory_optimization.png', dpi=300, bbox_inches='tight')
    plt.show()
