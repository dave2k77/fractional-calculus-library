...
from src.optimisation.jax_implementations import (
...
def gpu_acceleration_demo():
    ...
    methods = {
        ...
        'Riemann-Liouville': JAXFractionalDerivatives.riemann_liouville_derivative_gpu,
        'Grünwald-Letnikov': JAXFractionalDerivatives.grunwald_letnikov_derivative_gpu
    }
    ...
    print(f"Computing fractional derivatives (α = {alpha}) for f(t) = sin(t) * exp(-t/2)")
    ...
    for i, (method_name, result) in enumerate(results.items()):
        ...
        plt.plot(t, result, color=colors[i], linewidth=2, label=f'{method_name} (α={alpha})')
        plt.xlabel('Time t')
...
def automatic_differentiation_demo():
    ...
    try:
        ...
        grad_alpha = JAXAutomaticDifferentiation.gradient_wrt_alpha(
            lambda f, t, a, h: JAXFractionalDerivatives.caputo_derivative_gpu(f, t, a, h),
            f, t, alpha, h
        )
...
def vectorization_demo():
    ...
    try:
        ...
        vectorized_results = JAXVectorization.vectorize_over_alpha(
        ...
        for i, alpha in enumerate(alphas):
            plt.plot(t, vectorized_results[i], linewidth=2, 
        ...
        plt.tight_layout()
        plt.savefig('examples/jax_examples/vectorization_demo.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def performance_benchmark():
    ...
    for N in grid_sizes:
        ...
        methods = {
            'Caputo GPU': JAXFractionalDerivatives.caputo_derivative_gpu,
            'Riemann-Liouville GPU': JAXFractionalDerivatives.riemann_liouville_derivative_gpu,
            'Grünwald-Letnikov GPU': JAXFractionalDerivatives.grunwald_letnikov_derivative_gpu
        }
    ...
    for i, method in enumerate(methods):
        ...
        plt.loglog(grid_sizes, times, 'o-', color=colors[i], 
...
def fft_methods_demo():
    ...
    try:
        ...
        for method in methods:
            result = JAXFractionalDerivatives.fft_fractional_derivative_gpu(
                f, t, alpha, h, method
            )
        ...
        plt.tight_layout()
        plt.savefig('examples/jax_examples/fft_methods_demo.png', dpi=300, bbox_inches='tight')
        plt.show()
