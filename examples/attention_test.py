...
import numpy as np
from hpfracc.ml import BackendType, switch_backend, FractionalAttention
...
def test_attention():
    ...
    for backend in [BackendType.TORCH, BackendType.JAX, BackendType.NUMBA]:
        try:
            ...
            print(
                f"  ✅ Attention created with d_model={attention.d_model}, d_k={attention.d_k}")
            ...
            try:
                ...
                print(
                    f"  ✅ Forward pass successful, output shape: {output.shape}")

                # Test with different sequence lengths
                if backend == BackendType.TORCH:
                    x2 = torch.randn(1, 1, attention.d_model,
                                     dtype=torch.float32)
                elif backend == BackendType.JAX:
                    x2 = random.normal(random.PRNGKey(
                        1), (1, 1, attention.d_model))
                else:  # NUMBA
                    x2 = np.random.randn(
                        1, 1, attention.d_model).astype(np.float32)

                output2 = attention(x2, method="RL")
                print(
                    f"  ✅ Single sequence test successful, output shape: {output2.shape}")
