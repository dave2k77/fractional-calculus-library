...
def benchmark_backend_performance(
    ...
    """
    print(
        f"🔄 Testing {backend.value.upper()} backend with {model_type.upper()} model...")
    ...
    try:
        ...
        print(
            f"   ❌ {backend.value.upper()} + {model_type.upper()}: Failed - {str(e)}")
        return {
            'backend': backend.value,
            'model_type': model_type,
            'avg_time': float('inf'),
...
def visualize_performance_comparison(results: List[Dict[str, float]]) -> None:
    ...
    labels = [
        f"{r['backend'].upper()}-{r['model_type'].upper()}" for r in successful_results]
    ...
    plt.tight_layout()
    plt.savefig('fractional_gnn_performance_comparison.png',
    ...
    if all(f is not None for f in output_features):
        ...
        for i, (alpha, features) in enumerate(zip(fractional_orders, output_features)):
            if i < len(axes):
                ...
                ax.hist(features.flatten(), bins=30,
        ...
        plt.suptitle(
            f'Fractional Calculus Effects on {model_type.upper()} Outputs', fontsize=16)
        plt.tight_layout()
        plt.savefig(
            f'fractional_effects_{model_type}.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    ...
    print(
        f"   Graph: {node_features.shape[0]} nodes, {edge_index.shape[1]} edges")
    ...
    for backend in available_backends:
        for model_type in available_models:
            result = benchmark_backend_performance(
                backend, model_type, node_features, edge_index)
