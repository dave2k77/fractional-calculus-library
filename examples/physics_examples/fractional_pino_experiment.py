...
def generate_fractional_pde_data(
    ...
    x_tensor = torch.tensor(x, dtype=torch.float32).unsqueeze(
        0).repeat(n_samples, 1)
    y_tensor = torch.tensor(np.array(solutions),
...
def benchmark_fractional_pino(
    operator_types: list = ["laplacian",
                            "fourier", "riemann_liouville", "caputo"],
    ...
    for operator in operator_types:
        ...
        for alpha in fractional_orders:
            ...
            print(
                f"   Final validation loss: {final_val_metrics['total_loss']:.6f}")
...
def plot_benchmark_results(results: dict):
    ...
    for operator in operators:
        losses = [results[operator][alpha]['final_val_loss']
                  for alpha in alphas]
    ...
    ax3.plot(range(0, len(train_losses), 10), val_losses,
             'o-', label='Validation Loss', linewidth=2)
    ...
    with torch.no_grad():
        ...
        ax4.plot(x_test.numpy(), y_pred.numpy(), 'b-',
...
def compare_with_classical_methods(
    ...
    x = np.linspace(0, 1, n_points)
    def f(t): return np.sin(np.pi * t)  # Test function
    ...
    start_time = time.time()
    laplacian_result = fractional_laplacian(
        f, x, fractional_order, method="spectral")
    laplacian_time = time.time() - start_time

    # Fractional Fourier Transform
    start_time = time.time()
    u, fourier_result = fractional_fourier_transform(
        f, x, fractional_order, method="fast")
...
def main():
    ...
    print(
        f"   Final validation loss: {benchmark_results[best_operator][best_alpha]['final_val_loss']:.6f}")
    print(
