...
def L1_L2_schemes_comparison():
    ...
    if len(t_l2) > 1:
        ...
        f_interp = interp1d(t_l2, u_l2[:, idx], kind='linear', bounds_error=False, fill_value='extrapolate')
    ...
    if len(x_l2) > 1:
        from scipy.interpolate import interp1d
        f_interp = interp1d(x_l2, u_l2[idx, :], kind='linear', bounds_error=False, fill_value='extrapolate')
...
def predictor_corrector_demo():
    ...
    if len(t_pc) > 1 and len(y_pc) > 1:
        ...
        if min_len > 1:
            f_interp = interp1d(t_pc_trim, y_pc_trim, kind='linear', bounds_error=False, fill_value='extrapolate')
...
def anomalous_transport_demo():
    ...
    for alpha, x, t, u in solutions:
        ...
        if len(t) > 10:
            ...
            gamma = coeffs[0]
            ax_msd.loglog(t, msd, 'o-', linewidth=2, label=f'α = {alpha}, γ = {gamma:.2f}')
        else:
    ...
    X, T_mesh = np.meshgrid(x, t)
    contour1 = ax_contour.contour(X, T_mesh, solutions[sub_idx][3].T, levels=10, colors='blue', alpha=0.7)
    contour2 = ax_contour.contour(X, T_mesh, solutions[super_idx][3].T, levels=10, colors='red', alpha=0.7)
...
def levy_flights_demo():
        ...
        if len(right_tail) > 10:
            ...
            if peak_idx < len(right_tail) - 5:
                ...
                if np.sum(mask) > 5:
                    ...
                    exponent = -coeffs[0]  # Negative because we're fitting u ~ x^(-exponent)
                    ax_tail.loglog(tail_x[mask], tail_y[mask], 'o-', linewidth=2, 
    ...
    X, T_mesh = np.meshgrid(x, t)
    contour1 = ax_contour.contour(X, T_mesh, solutions[levy_idx][3].T, levels=10, colors='blue', alpha=0.7)
    contour2 = ax_contour.contour(X, T_mesh, solutions[normal_idx][3].T, levels=10, colors='red', alpha=0.7)
    ax_contour.set_xlabel('Position x')
    ax_contour.set_ylabel('Time t')
    ...
    for alpha, beta, description, x, t, u in solutions:
        if alpha < 1:  # Heavy-tailed waiting times
            ...
            try:
                ...
                ax_waiting.loglog(t_waiting, waiting_dist, linewidth=2, label=f'α = {alpha}')
            except:
