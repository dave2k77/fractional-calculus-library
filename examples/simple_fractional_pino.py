...
class SimpleFractionalPINO(nn.Module):
    ...
    def apply_fractional_operator(self, x, alpha=None):
        ...
        if self.operator_type == "laplacian":
            ...
            result = fractional_laplacian_spectral(
                f, np.linspace(0, 1, len(x_np)), alpha)
            ...
            u, result = fractional_fourier_transform_fast(
                f, np.linspace(0, 1, len(x_np)), alpha)
...
def generate_fractional_pde_data(n_points=100, n_samples=50):
    ...
    x_tensor = torch.tensor(
        x, dtype=torch.float32).unsqueeze(-1)  # (n_points, 1)
    # (n_samples, n_points)
    y_tensor = torch.tensor(np.array(solutions), dtype=torch.float32)
...
def train_fractional_pino(operator_type="laplacian", fractional_order=0.5, epochs=100):
    ...
    for epoch in range(epochs):
        ...
        for i in range(y.shape[0]):  # For each sample
            ...
            # Reconstruction loss
            recon_loss_sample = criterion(
                y_pred_sample.squeeze(), y_true_sample)
            total_recon_loss += recon_loss_sample

            # Physics loss: fractional operator constraint
            frac_op_pred = model.apply_fractional_operator(
                y_pred_sample, fractional_order)
            lambda_param = 1.0
            physics_residual = frac_op_pred + lambda_param * y_pred_sample
            physics_loss_sample = criterion(
                physics_residual, torch.zeros_like(physics_residual))
    ...
    with torch.no_grad():
        ...
        final_recon_loss = criterion(y_pred.squeeze(), y[0]).item()
...
def plot_training_results(results, operator_type, fractional_order):
    ...
    fig.suptitle(
        f'Fractional PINO Training Results ({operator_type}, α={fractional_order})', fontsize=16)

    # Plot 1: Training losses
    axes[0, 0].plot(results['losses'], 'b-', linewidth=2, label='Total Loss')
    axes[0, 0].plot(results['recon_losses'], 'r-',
                    linewidth=2, label='Reconstruction Loss')
    axes[0, 0].plot(results['physics_losses'], 'g-',
                    linewidth=2, label='Physics Loss')
    ...
    with torch.no_grad():
        frac_op_pred = model.apply_fractional_operator(
            results['y_pred'][sample_idx:sample_idx+1])
        physics_residual = frac_op_pred + \
            results['y_pred'][sample_idx:sample_idx+1]

    axes[1, 0].plot(
        x_sample, physics_residual.numpy().flatten(), 'g-', linewidth=2)
    ...
    print(
        f"✅ Results saved to 'fractional_pino_{operator_type}_alpha_{fractional_order}_results.png'")
...
def benchmark_fractional_pino():
    ...
    for operator in operators:
        ...
        for alpha in alphas:
            ...
            print(
                f"✅ {operator} (α={alpha}) completed in {result['training_time']:.2f}s")
...
def plot_benchmark_results(benchmark_results):
    ...
    ax1 = axes[0, 0]
    for operator in operators:
        losses = [benchmark_results[operator][alpha]['final_loss']
                  for alpha in alphas]
        ax1.plot(alphas, losses, 'o-', label=operator,
    ...
    ax2 = axes[0, 1]
    for operator in operators:
        times = [benchmark_results[operator][alpha]['training_time']
                 for alpha in alphas]
        ax2.bar([f"{operator}\nα={alpha}" for alpha in alphas],
                times, alpha=0.7)
    ...
    best_alpha = min(
        alphas, key=lambda a: benchmark_results[best_operator][a]['final_loss'])
    ...
    plt.tight_layout()
    plt.savefig('fractional_pino_benchmark_results.png',
                dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print(
        f"   Final loss: {benchmark_results[best_operator][best_alpha]['final_loss']:.6f}")
    print(
        f"   Training time: {benchmark_results[best_operator][best_alpha]['training_time']:.2f}s")
