...
def demo_parallel_methods():
    ...
    try:
        ...
        parallel_rl = create_fractional_derivative('parallel_riemann_liouville', alpha)
        parallel_rl_result = parallel_rl.compute(f, x)
        print(f"Parallel Riemann-Liouville D^{alpha}f(x): shape {parallel_rl_result.shape}")
    ...
    try:
        # Parallel Caputo
        parallel_caputo = create_fractional_derivative('parallel_caputo', alpha)
        parallel_caputo_result = parallel_caputo.compute(f, x)
        print(f"Parallel Caputo D^{alpha}f(x): shape {parallel_caputo_result.shape}")
...
def demo_performance_comparison():
    ...
    classical_derivative = create_fractional_derivative('riemann_liouville', alpha)
    classical_result = classical_derivative.compute(f, x)
    ...
    try:
        start_time = time.time()
        parallel_derivative = create_fractional_derivative('parallel_riemann_liouville', alpha)
        parallel_result = parallel_derivative.compute(f, x)
...
def create_visualization():
    ...
    try:
        ...
        if len(rl_result) != len(x):
            rl_result = rl_result[:len(x)] if len(rl_result) > len(x) else np.pad(rl_result, (0, len(x) - len(rl_result)))
    ...
    try:
        ...
        if len(caputo_result) != len(x):
            caputo_result = caputo_result[:len(x)] if len(caputo_result) > len(x) else np.pad(caputo_result, (0, len(x) - len(caputo_result)))
    ...
    try:
        ...
        if len(gl_result) != len(x):
            gl_result = gl_result[:len(x)] if len(gl_result) > len(x) else np.pad(gl_result, (0, len(x) - len(gl_result)))
    ...
    try:
        ...
        if len(rf_result) != len(x):
            rf_result = rf_result[:len(x)] if len(rf_result) > len(x) else np.pad(rf_result, (0, len(x) - len(rf_result)))
    ...
    for name, result in results.items():
        try:
            plt.plot(x, result, '--', linewidth=2, label=f'{name} D^{alpha}f(x)')
        except Exception as e:
    ...
    # Save plot
    plt.savefig('fractional_derivatives_comparison.png', dpi=300, bbox_inches='tight')
