...
class FractionalDiffusionSolver(FractionalPDESolver):
    ...
    def solve(self, 
        ...
        for n in range(1, nt):
            ...
            solution[n, 1:-1] = self._solve_spatial_step(
                solution, n, x_values, t_n, alpha, beta, diffusion_coeff, 
                source_term, dx, dt, **kwargs
            )
        
        return t_values, x_values, solution
    
    def _solve_spatial_step(self, 
                           solution: np.ndarray,
                           n: int,
                           x_values: np.ndarray,
                           t_n: float,
    ...
    def _compute_spatial_derivative(self, 
                                  u: np.ndarray,
                                  beta: Union[float, FractionalOrder],
                                  dx: float) -> np.ndarray:
        """
    ...
    def _build_spatial_matrix(self, 
                            ...
                            dx: float,
                            dt: float,
                            alpha: Union[float, FractionalOrder]) -> sparse.spmatrix:
        """
        ...
        # Main diagonal
        main_diag = np.ones(n_interior) * (1 + 2 * diffusion_coeff * dt ** alpha_val / (dx ** 2))
        diagonals.append(main_diag)
        offsets.append(0)
        
        # Sub-diagonal
        sub_diag = np.ones(n_interior - 1) * (-diffusion_coeff * dt ** alpha_val / (dx ** 2))
        diagonals.append(sub_diag)
        offsets.append(-1)
        
        # Super-diagonal
        super_diag = np.ones(n_interior - 1) * (-diffusion_coeff * dt ** alpha_val / (dx ** 2))
...
def solve_fractional_diffusion(x_span: Tuple[float, float],
                             t_span: Tuple[float, float],
                             initial_condition: Callable,
                             boundary_conditions: Tuple[Callable, Callable],
                             alpha: Union[float, FractionalOrder],
...
def solve_fractional_advection(x_span: Tuple[float, float],
                             t_span: Tuple[float, float],
                             initial_condition: Callable,
                             boundary_conditions: Tuple[Callable, Callable],
                             alpha: Union[float, FractionalOrder],
...
def solve_fractional_reaction_diffusion(x_span: Tuple[float, float],
                                      t_span: Tuple[float, float],
                                      initial_condition: Callable,
                                      boundary_conditions: Tuple[Callable, Callable],
                                      alpha: Union[float, FractionalOrder],
    ...
    return solver.solve(
        x_span, t_span, initial_condition, boundary_conditions,
        alpha, beta, diffusion_coeff, reaction_term, source_term, nx, nt, **kwargs
    )
