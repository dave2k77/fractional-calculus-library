...
class AdvancedFractionalODESolver:
    ...
    def solve(
        ...
        **kwargs,
        ...
        while current_t < tf and iteration < self.max_iter:
            ...
            if self._is_step_acceptable(error_est, current_y, next_y):
                ...
                if current_h <= self.min_step and len(error_estimates) > 0:
                    ...
                    warnings.warn(f"Step size at minimum ({self.min_step}), forcing acceptance")
                    t_values.append(next_t)
                    y_values.append(next_y)
                    h_values.append(current_h)
                    error_estimates.append(error_est)
        ...
        if len(t_values) < 2:
            warnings.warn("Advanced solver failed to make progress, falling back to basic solver")
            from src.solvers.ode_solvers import solve_fractional_ode
            t_basic, y_basic = solve_fractional_ode(f, t_span, y0, alpha, method="predictor_corrector")
            return {
                't': t_basic,
                'y': y_basic,
                'h': np.diff(t_basic),
...
class HighOrderFractionalSolver:
    ...
    def _multistep_solve(
        ...
        **kwargs,
        ...
        y0_array = np.array(y0)
        y_values = np.zeros((len(t_values), 1 if y0_array.ndim == 0 else y0_array.shape[0]))
        y_values[0] = y0
    ...
    def _chebyshev_differentiation_matrix(self, n: int) -> np.ndarray:
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    D[i, j] = (-1)**(i+j) / (np.cos(i*np.pi/(n-1)) - np.cos(j*np.pi/(n-1)))
                else:
                    D[i, i] = 0
    ...
    def _multistep_step(
        ...
        if self.order == 2:
            return y[i-1] + h * (1.5 * f(t[i-1], y[i-1]) - 0.5 * f(t[i-2], y[i-2]))
        elif self.order == 3:
            return y[i-1] + h * (23/12 * f(t[i-1], y[i-1]) - 16/12 * f(t[i-2], y[i-2]) + 5/12 * f(t[i-3], y[i-3]))
        else:
