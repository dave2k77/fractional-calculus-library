...
from src.algorithms import (
    caputo_derivative, riemann_liouville_derivative, grunwald_letnikov_derivative
)
...
class FractionalODESolver:
    ...
    def __init__(self, 
        ...
        # Validate method
        valid_methods = ["predictor_corrector", "adams_bashforth", "runge_kutta", "euler"]
    ...
    def _solve_predictor_corrector(self, 
        ...
        for n in range(1, N):
            ...
            y_corr = self._corrector_step(f, t_values, y_values, y_pred, n, alpha, coeffs, h)
    ...
    def _solve_adams_bashforth(self, 
                              f: Callable,
                              t0: float,
                              tf: float,
                              y0: Union[float, np.ndarray],
        ...
        for n in range(1, N):
            ...
            y_values[n] = self._adams_bashforth_step(f, t_values, y_values, n, alpha, coeffs, h)
    ...
    def _compute_fractional_coefficients(self, 
                                       alpha: Union[float, FractionalOrder],
                                       N: int) -> np.ndarray:
        """
        Compute fractional derivative coefficients.
        ...
        for j in range(1, N):
            if self.derivative_type == "caputo":
                ...
                coeffs[j] = (-1) ** j * gamma(alpha_val + 1) / (gamma(j + 1) * gamma(alpha_val - j + 1))
    ...
    def _adams_bashforth_step(self, 
                            f: Callable,
                            t_values: np.ndarray,
                            y_values: np.ndarray,
                            n: int,
    ...
    def _runge_kutta_step(self, 
        ...
        # Runge-Kutta formula
        y_next = y_n + (h ** alpha_val / gamma(alpha_val + 1)) * (k1 + 2*k2 + 2*k3 + k4) / 6
...
class AdaptiveFractionalODESolver(FractionalODESolver):
    ...
    def solve(self, 
        ...
        while t_current < tf:
            ...
            y_next = self._adaptive_step(f, t_current, t_next, y_current, alpha, h_actual)
            
            # Estimate error
            error = self._estimate_error(f, t_current, t_next, y_current, y_next, alpha, h_actual)
    ...
    def _adaptive_step(self, 
        ...
        """
        if self.method == "predictor_corrector":
            return self._adaptive_predictor_corrector(f, t_current, t_next, y_current, alpha, h)
        elif self.method == "runge_kutta":
            return self._adaptive_runge_kutta(f, t_current, t_next, y_current, alpha, h)
        else:
            # Fall back to non-adaptive method
            return self._euler_step(f, np.array([t_current, t_next]), 
                                  np.array([y_current, y_current]), 1, alpha, 
                                  self._compute_fractional_coefficients(alpha, 2), h)
    
    def _adaptive_predictor_corrector(self, 
                                    f: Callable,
                                    t_current: float,
                                    t_next: float,
                                    y_current: np.ndarray,
    ...
    def _adaptive_runge_kutta(self, 
                            f: Callable,
                            t_current: float,
                            t_next: float,
                            y_current: np.ndarray,
        ...
        # Runge-Kutta formula
        y_next = y_current + (h ** alpha / gamma(alpha + 1)) * (k1 + 2*k2 + 2*k3 + k4) / 6
...
def solve_fractional_ode(f: Callable,
                        t_span: Tuple[float, float],
                        y0: Union[float, np.ndarray],
                        alpha: Union[float, FractionalOrder],
                        derivative_type: str = "caputo",
...
def solve_fractional_system(f: Callable,
                          t_span: Tuple[float, float],
                          y0: np.ndarray,
