...
class PredictorCorrectorSolver:
    ...
    def _solve_fixed_step(self, 
        ...
        for n in range(1, N):
            ...
            y_corr = self._corrector_step(f, t_values, y_values, y_pred, n, alpha, coeffs, h0)
    ...
    def _solve_adaptive(self, 
        ...
        while t_current < tf:
            ...
            # Predictor step
            y_pred = self._adaptive_predictor_step(f, t_current, t_next, y_current, alpha, h_actual)
            
            # Corrector step
            y_corr = self._adaptive_corrector_step(f, t_current, t_next, y_current, y_pred, alpha, h_actual)
    ...
    def _adaptive_predictor_step(self, 
                               f: Callable,
                               t_current: float,
                               t_next: float,
        ...
        # Simple predictor
        y_pred = y_current + (h ** alpha_val / gamma(alpha_val + 1)) * f(t_current, y_current)
        
        return y_pred
    
    def _adaptive_corrector_step(self, 
                               f: Callable,
                               t_current: float,
                               t_next: float,
                               y_current: np.ndarray,
...
class AdamsBashforthMoultonSolver(PredictorCorrectorSolver):
    ...
    def _predictor_step(self, 
        ...
        for j in range(n):
            t_j = t_values[j]
            y_pred += h ** alpha_val / gamma(alpha_val + 1) * weights[j] * f(t_j, y_values[j])
    ...
    def _corrector_step(self, 
        ...
        for j in range(n):
            t_j = t_values[j]
            if j == n - 1:
                y_corr += h ** alpha_val / gamma(alpha_val + 1) * weights[j] * f(t_j, y_pred)
            else:
                y_corr += h ** alpha_val / gamma(alpha_val + 1) * weights[j] * f(t_j, y_values[j])
...
class VariableStepPredictorCorrector(PredictorCorrectorSolver):
    ...
    def _adaptive_step_size_control(self, 
        ...
        if error <= self.tol:
            ...
            factor = min(2.0, self.safety_factor * (self.tol / error) ** (1.0 / (order + 1)))
            h_new = min(self.max_h, h_current * factor)
        else:
            # Reject step and decrease step size
            factor = max(0.1, self.safety_factor * (self.tol / error) ** (1.0 / (order + 1)))
...
def solve_predictor_corrector(f: Callable,
                            t_span: Tuple[float, float],
                            y0: Union[float, np.ndarray],
                            alpha: Union[float, FractionalOrder],
                            derivative_type: str = "caputo",
...
def solve_adams_bashforth_moulton(f: Callable,
                                t_span: Tuple[float, float],
                                y0: Union[float, np.ndarray],
                                alpha: Union[float, FractionalOrder],
                                derivative_type: str = "caputo",
...
def solve_variable_step_predictor_corrector(f: Callable,
                                          t_span: Tuple[float, float],
                                          y0: Union[float, np.ndarray],
                                          alpha: Union[float, FractionalOrder],
                                          derivative_type: str = "caputo",
