...
class ConvergenceTester:
    ...
    def test_convergence(self, method_func: Callable, analytical_func: Callable,
                        grid_sizes: List[int], test_params: Dict,
                        error_norm: str = 'l2') -> Dict:
        """
        ...
        if len(valid_indices) < 2:
            raise ValueError("Need at least 2 valid error measurements for convergence analysis")
        
        valid_grid_sizes = [grid_sizes[i] for i in valid_indices]
        valid_errors = [errors[i] for i in valid_indices]
        
        # Compute convergence rate
        convergence_rate = self._compute_convergence_rate(valid_grid_sizes, valid_errors)
    ...
    def _compute_convergence_rate(self, grid_sizes: List[int], errors: List[float]) -> float:
        """
    ...
    def test_multiple_norms(self, method_func: Callable, analytical_func: Callable,
                           grid_sizes: List[int], test_params: Dict) -> Dict:
        """
...
class ConvergenceAnalyzer:
    ...
    def analyze_method_convergence(self, method_func: Callable, analytical_func: Callable,
        ...
        if convergence_rates:
            results['summary'] = {
                ...
                'successful_test_cases': len([c for c in results['test_cases'] if c['success']]),
            }
        else:
            results['summary'] = {
    ...
    def estimate_optimal_grid_size(self, target_error: float, convergence_rate: float,
                                  reference_grid_size: int, reference_error: float) -> int:
        """
        ...
        N_opt = int(reference_grid_size * (reference_error / target_error) ** (1 / convergence_rate))
        
        return max(N_opt, 1)  # Ensure positive grid size
    
    def validate_convergence_order(self, observed_rate: float, expected_order: OrderOfAccuracy,
...
def run_convergence_study(method_func: Callable, analytical_func: Callable,
                         test_cases: List[Dict], grid_sizes: List[int] = None) -> Dict:
    """
    ...
    return analyzer.analyze_method_convergence(method_func, analytical_func, test_cases, grid_sizes)


def run_method_convergence_test(method_func: Callable, analytical_func: Callable,
                           grid_sizes: List[int], test_params: Dict) -> Dict:
    """
    Test convergence of a numerical method.
    ...
    return tester.test_multiple_norms(method_func, analytical_func, grid_sizes, test_params)
