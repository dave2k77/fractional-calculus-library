...
class L1L2Schemes:
    ...
    def solve_time_fractional_pde(
        ...
        **kwargs,
        ...
        if self.scheme == "l1":
            ...
            return self._solve_l2_1_sigma_scheme(
                initial_condition,
                boundary_conditions,
                alpha,
                t_final,
            ...
            return self._solve_l2_1_theta_scheme(
                initial_condition,
                boundary_conditions,
                alpha,
                t_final,
    ...
    def _compute_l2_coefficients(self, alpha: float, N: int) -> np.ndarray:
        ...
        coeffs[0] = 1.0
        for j in range(1, N):
            # Handle potential complex results from power operations
            term1 = (j + 1) ** alpha
            term2 = j**alpha
            term3 = (j - 1) ** alpha

            # Take real part if complex
            if hasattr(term1, "real"):
                term1 = term1.real
            if hasattr(term2, "real"):
                term2 = term2.real
            if hasattr(term3, "real"):
                term3 = term3.real

            coeffs[j] = term1 - 2 * term2 + term3
        return coeffs

    def _compute_l2_1_sigma_coefficients(
        self, alpha: float, sigma: float, N: int
    ) -> np.ndarray:
        ...
        for j in range(1, N):
            # Handle potential complex results from power operations
            term1 = (j + sigma) ** alpha
            term2 = (j + sigma - 1) ** alpha
            term3 = (j + sigma - 2) ** alpha
    ...
    def _compute_l2_1_theta_coefficients(
        self, alpha: float, theta: float, N: int
    ) -> np.ndarray:
        ...
        for j in range(1, N):
            # Handle potential complex results from power operations
            term1 = (j + theta) ** alpha
            term2 = (j + theta - 1) ** alpha
            term3 = (j + theta - 2) ** alpha
...
class JAXL1L2Schemes:
    ...
    @jax.jit
    def l1_step_jax(
        u_prev: jnp.ndarray, coeffs: jnp.ndarray, A: jnp.ndarray, rhs: jnp.ndarray
    ) -> jnp.ndarray:
        """
...
def l2_scheme_numba(
    ...
    # Use L1 for first step
    if N_t > 1:
        u[1, :] = l1_scheme_numba(initial_condition, alpha, dt, dx, diffusion_coeff, 2)[
            1, :
        ]
...
def solve_time_fractional_pde(
    ...
    **kwargs,
    ...
    return solver.solve_time_fractional_pde(
        initial_condition,
        boundary_conditions,
        alpha,
        t_final,
...
def solve_time_fractional_pde_numba(
    initial_condition: np.ndarray,
    alpha: Union[float, FractionalOrder],
    t_final: float,
    dt: float,
    ...
    if scheme == "l1":
        return l1_scheme_numba(
            initial_condition, alpha_val, dt, dx, diffusion_coeff, N_t
        )
    elif scheme == "l2":
        return l2_scheme_numba(
            initial_condition, alpha_val, dt, dx, diffusion_coeff, N_t
        )
    else:
