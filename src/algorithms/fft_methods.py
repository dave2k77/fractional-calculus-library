...
class FFTFractionalMethods:
    ...
    def compute_derivative(self, f: np.ndarray, t: np.ndarray, 
    ...
    def compute_integral(self, f: np.ndarray, t: np.ndarray,
                        alpha: Union[float, FractionalOrder],
                        **kwargs) -> np.ndarray:
        """
    ...
    def _convolution_derivative(self, f: np.ndarray, t: np.ndarray,
                               alpha: Union[float, FractionalOrder],
                               **kwargs) -> np.ndarray:
        """
        ...
        for i in range(N):
            if i == 0:
                ...
                kernel[i] = (t[i] ** (-alpha_val - 1)) / gamma(-alpha_val)
    ...
    def _spectral_derivative(self, f: np.ndarray, t: np.ndarray,
                            alpha: Union[float, FractionalOrder],
                            **kwargs) -> np.ndarray:
        """
    ...
    def _fractional_fourier_derivative(self, f: np.ndarray, t: np.ndarray,
                                      alpha: Union[float, FractionalOrder],
                                      **kwargs) -> np.ndarray:
        """
    ...
    def _fractional_fourier_transform(self, f: np.ndarray, phi: float) -> np.ndarray:
        ...
        for m in range(N):
            for n in range(N):
                if phi != 0:
                    kernel[m, n] = np.exp(1j * phi) * np.sqrt(1j / (2 * np.pi * np.sin(phi))) * \
                                  np.exp(1j * ((m**2 + n**2) * np.cos(phi) - 2*m*n) / (2 * np.sin(phi)))
                else:
    ...
    def _inverse_fractional_fourier_transform(self, f: np.ndarray, phi: float) -> np.ndarray:
        """
    ...
    def _wavelet_derivative(self, f: np.ndarray, t: np.ndarray,
                           alpha: Union[float, FractionalOrder],
                           **kwargs) -> np.ndarray:
        """
...
class JAXFFTFractionalMethods:
    ...
    @staticmethod
    @jax.jit
    def convolution_derivative_jax(f_values: jnp.ndarray, t_values: jnp.ndarray,
    ...
    @staticmethod
    @jax.jit
    def spectral_derivative_jax(f_values: jnp.ndarray, t_values: jnp.ndarray,
                               alpha: float, h: float) -> jnp.ndarray:
        """
...
def fft_convolution_derivative_numba(f: np.ndarray, t: np.ndarray, 
                                    alpha: float, h: float) -> np.ndarray:
    """
    ...
    kernel = np.zeros(N)
    for i in range(1, N):
        kernel[i] = (t[i] ** (-alpha - 1)) / gamma(-alpha)
...
@jit(nopython=True, parallel=True)
def fft_spectral_derivative_numba(f: np.ndarray, t: np.ndarray,
                                 alpha: float, h: float) -> np.ndarray:
    """
...
def fft_fractional_derivative(f: np.ndarray, t: np.ndarray,
                             alpha: Union[float, FractionalOrder],
...
def fft_fractional_integral(f: np.ndarray, t: np.ndarray,
                           alpha: Union[float, FractionalOrder],
                           method: str = "convolution",
                           **kwargs) -> np.ndarray:
    """
...
def fft_fractional_derivative_jax(f_values: np.ndarray, t_values: np.ndarray,
                                 alpha: Union[float, FractionalOrder], h: float,
                                 method: str = "convolution") -> np.ndarray:
    """
    ...
    if method == "convolution":
        ...
        raise ValueError("Method must be 'convolution' or 'spectral' for JAX implementation")


def fft_fractional_derivative_numba(f: np.ndarray, t: np.ndarray,
                                   alpha: Union[float, FractionalOrder],
                                   method: str = "convolution") -> np.ndarray:
    """
    ...
    if method == "convolution":
        ...
        raise ValueError("Method must be 'convolution' or 'spectral' for NUMBA implementation")
