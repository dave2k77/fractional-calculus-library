...
class WeylDerivative:
    ...
    def _compute_parallel(self, f: np.ndarray, x: np.ndarray, h: float) -> np.ndarray:
        ...
        chunks = [(f[i:i+chunk_size], x[i:i+chunk_size], h) 
    ...
    def _process_chunk(self, chunk_data: Tuple[np.ndarray, np.ndarray, float]) -> np.ndarray:
...
class MarchaudDerivative:
    ...
    def _compute_memory_optimized(self, f: np.ndarray, x: np.ndarray, h: float, 
                                 use_parallel: bool) -> np.ndarray:
    ...
    def _process_marchaud_chunk(self, chunk_data: Tuple[np.ndarray, np.ndarray, float, int, int]) -> np.ndarray:
    ...
    def _compute_marchaud_segment(self, f: np.ndarray, x: np.ndarray, h: float, 
    ...
    def _compute_standard(self, f: np.ndarray, x: np.ndarray, h: float, 
                         use_parallel: bool) -> np.ndarray:
...
class ReizFellerDerivative:
    ...
    def _compute_spectral(self, f: np.ndarray, x: np.ndarray, h: float, 
                         use_parallel: bool) -> np.ndarray:
...
class AdomianDecomposition:
    ...
    def solve(self, equation: Callable, initial_conditions: Dict, 
              ...
              n_terms: int = 10, use_parallel: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _compute_terms_serial(self, equation: Callable, t: np.ndarray, h: float, 
                             n_terms: int) -> List[np.ndarray]:
    ...
    def _compute_terms_parallel(self, equation: Callable, t: np.ndarray, h: float, 
                               n_terms: int) -> List[np.ndarray]:
    ...
    def _compute_single_term(self, equation: Callable, t: np.ndarray, h: float, n: int) -> np.ndarray:
    ...
    def _compute_adomian_polynomial(self, equation: Callable, previous_terms: List[np.ndarray], 
                                   n: int) -> np.ndarray:
    ...
    def _compute_integral_term(self, adomian: np.ndarray, t: np.ndarray, h: float) -> np.ndarray:
...
def weyl_derivative(f: Union[Callable, np.ndarray], x: Union[float, np.ndarray], 
...
def marchaud_derivative(f: Union[Callable, np.ndarray], x: Union[float, np.ndarray], 
                       alpha: float, h: Optional[float] = None, **kwargs) -> Union[float, np.ndarray]:
...
def hadamard_derivative(f: Union[Callable, np.ndarray], x: Union[float, np.ndarray], 
                       alpha: float, h: Optional[float] = None, **kwargs) -> Union[float, np.ndarray]:
...
def reiz_feller_derivative(f: Union[Callable, np.ndarray], x: Union[float, np.ndarray], 
                          alpha: float, h: Optional[float] = None, **kwargs) -> Union[float, np.ndarray]:
