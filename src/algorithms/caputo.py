...
class CaputoDerivative:
    ...
    def _compute_direct_scalar(self, f: Union[Callable, np.ndarray], 
        ...
        if callable(f):
            ...
            def integrand(tau):
                return (t - tau) ** (n - alpha - 1) * self._nth_derivative(f_interp, tau, n)
    ...
    def _compute_l1(self, f: Union[Callable, np.ndarray], 
                   t: Union[float, np.ndarray],
                   h: float, **kwargs) -> Union[float, np.ndarray]:
        """
    ...
    def _compute_l2(self, f: Union[Callable, np.ndarray], 
                   t: Union[float, np.ndarray],
                   h: float, **kwargs) -> Union[float, np.ndarray]:
        """
    ...
    def _l2_scheme(self, f: np.ndarray, t: np.ndarray, h: float) -> np.ndarray:
        ...
        # Compute derivative
        for n in range(2, N):
            result[n] = (h ** (-alpha) / gamma(3 - alpha)) * np.sum(
                coeffs[:n+1] * f[n-j]
            )
    ...
    def _nth_derivative(self, f: Callable, t: float, n: int) -> float:
        ...
        if n == 0:
            ...
            h = 1e-6
            return (self._nth_derivative(f, t + h, n - 1) - 
                   self._nth_derivative(f, t - h, n - 1)) / (2 * h)
...
def caputo_derivative(f: Union[Callable, np.ndarray], 
                     t: Union[float, np.ndarray],
...
def caputo_derivative_jax(f_values: np.ndarray, t_values: np.ndarray,
...
def caputo_derivative_numba(f: np.ndarray, t: np.ndarray,
                           alpha: Union[float, FractionalOrder],
