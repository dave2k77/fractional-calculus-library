...
class GrunwaldLetnikovDerivative:
    ...
    def __init__(self, alpha: Union[float, FractionalOrder], 
        ...
        # Validate method
        valid_methods = ["direct", "fft", "short_memory", "variable_step", "predictor_corrector"]
    ...
    def _compute_direct_vectorized(self, f: Union[Callable, np.ndarray], 
                                  t: np.ndarray, h: float, **kwargs) -> np.ndarray:
    ...
    def _grunwald_coefficient(self, alpha: float, j: int) -> float:
        ...
        if j == 0:
            ...
            return (-1) ** j * gamma(alpha + 1) / (gamma(j + 1) * gamma(alpha - j + 1))
    ...
    def _compute_short_memory(self, f: Union[Callable, np.ndarray], 
                             t: Union[float, np.ndarray],
                             h: float, **kwargs) -> Union[float, np.ndarray]:
        """
    ...
    def _short_memory_scheme(self, f: np.ndarray, t: np.ndarray, h: float, **kwargs) -> np.ndarray:
    ...
    def _compute_variable_step(self, f: Union[Callable, np.ndarray], 
                              t: Union[float, np.ndarray],
                              h: float, **kwargs) -> Union[float, np.ndarray]:
        """
    ...
    def _variable_step_scheme(self, f: np.ndarray, t: np.ndarray, h: float, **kwargs) -> np.ndarray:
    ...
    def compute_jax(f_values: jnp.ndarray, t_values: jnp.ndarray, 
        ...
        def compute_coeff(j):
            return jax.lax.cond(
                j == 0,
                lambda _: 1.0,
                lambda _: (-1) ** j * gamma(alpha + 1) / (gamma(j + 1) * gamma(alpha - j + 1)),
                operand=None
            )
...
@jit(nopython=True, parallel=True)
def grunwald_letnikov_short_memory_numba(f: np.ndarray, alpha: float, h: float, 
...
def grunwald_letnikov_derivative(f: Union[Callable, np.ndarray], 
                                t: Union[float, np.ndarray],
...
def grunwald_letnikov_derivative_jax(f_values: np.ndarray, t_values: np.ndarray,
...
def grunwald_letnikov_derivative_numba(f: np.ndarray, t: np.ndarray,
                                      alpha: Union[float, FractionalOrder],
                                      method: str = "direct",
                                      **kwargs) -> np.ndarray:
    """
    ...
    if method == "direct":
        ...
        raise ValueError("Method must be 'direct' or 'short_memory' for NUMBA implementation")
