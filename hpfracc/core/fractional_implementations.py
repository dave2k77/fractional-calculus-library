...
class RiemannLiouvilleDerivative(BaseFractionalDerivative):
    ...
    def compute(self, f: Callable, x: Union[float, np.ndarray], **kwargs) -> Union[float, np.ndarray]:
        """Compute the Riemann-Liouville fractional derivative."""
        return self._optimized_impl.compute(f, x, **kwargs)
    
    def compute_numerical(self, f_values: np.ndarray, x_values: np.ndarray, **kwargs) -> np.ndarray:
...
class CaputoFabrizioDerivative(BaseFractionalDerivative):
    ...
    def compute(self, f: Callable, x: Union[float, np.ndarray], **kwargs) -> Union[float, np.ndarray]:
        ...
        if isinstance(x, (int, float)) or (isinstance(x, np.ndarray) and x.size == 1):
...
class FractionalLaplacian(BaseFractionalDerivative):
    ...
    def compute_numerical(self, f_values: np.ndarray, x_values: np.ndarray, **kwargs) -> np.ndarray:
        ...
        return self._special_impl.compute_numerical(f_values, x_values, **kwargs)
...
class ParallelOptimizedRiemannLiouville(BaseFractionalDerivative):
    ...
    def compute_numerical(self, f_values: np.ndarray, x_values: np.ndarray, **kwargs) -> np.ndarray:
        ...
        return self._parallel_impl.compute_numerical(f_values, x_values, **kwargs)
...
class RieszFisherOperator(BaseFractionalDerivative):
    ...
    def __init__(self, alpha: Union[float, FractionalOrder], **kwargs):
        ...
        self._original_alpha = alpha if isinstance(alpha, (int, float)) else alpha.alpha
        ...
        if self._use_derivative:
            from .fractional_implementations import RiemannLiouvilleDerivative
            self._left_op = RiemannLiouvilleDerivative(abs(self._original_alpha))
            self._right_op = RiemannLiouvilleDerivative(abs(self._original_alpha))
        elif self._use_integral:
            from .fractional_implementations import create_fractional_integral
            self._left_op = create_fractional_integral("RL", abs(self._original_alpha))
            self._right_op = create_fractional_integral("RL", abs(self._original_alpha))
        else:
    ...
    def compute_numerical(self, f_values: np.ndarray, x_values: np.ndarray, **kwargs) -> np.ndarray:
        ...
        # For α ≠ 0, compute left and right operators
        if self._use_derivative:
            left_result = self._left_op.compute_numerical(f_values, x_values, **kwargs)
            right_result = self._right_op.compute_numerical(f_values, x_values, **kwargs)
        else:  # self._use_integral
...
class AdomianDecompositionMethod:
    ...
    def solve(self, equation: Callable, initial_condition: Callable, 
        ...
        adomian_solver = AdomianDecomposition(max_terms=self.max_terms, tolerance=self.tolerance)
        return adomian_solver.solve(equation, initial_condition, domain, **kwargs)

    def compute_adomian_polynomials(self, nonlinear_term: Callable, u_series: List[Callable], 
        ...
        from ..algorithms.advanced_methods import AdomianDecomposition
        adomian_solver = AdomianDecomposition(max_terms=self.max_terms, tolerance=self.tolerance)
        return adomian_solver.compute_adomian_polynomials(nonlinear_term, u_series, order)


def create_fractional_integral(method: str, alpha: Union[float, FractionalOrder], **kwargs):
    """
...
def create_riesz_fisher_operator(alpha: Union[float, FractionalOrder], **kwargs):
    """
