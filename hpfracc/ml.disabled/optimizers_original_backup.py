...
class SimpleFractionalOptimizer(ABC):
    ...
    def zero_grad(self, params: Optional[List[Any]] = None) -> None:
        ...
        if params is None:
            # This optimizer doesn't track all parameters, so we can't zero gradients
            # without knowing which parameters to zero. This is a limitation of the
            # simplified design. Users should pass parameters explicitly.
            raise ValueError("This optimizer requires parameters to be passed to zero_grad(). "
                           "Use zero_grad(net.parameters()) instead.")
        else:
            # Zero gradients for the provided parameters
            for param in params:
...
class SimpleFractionalSGD(SimpleFractionalOptimizer):
    ...
    def step(self, params: List[Any], gradients: Optional[List[Any]] = None) -> None:
        ...
        for param, grad in zip(params, gradients):
            ...
            # Apply fractional derivative to gradients if enabled
