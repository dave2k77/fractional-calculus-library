...
class FractionalConv1D:
    ...
    def __init__(
        ...
        self.groups = groups
        # Keep boolean flag accessible via `bias` for constructor assertions
        self.bias_flag = bool(bias)
        self._backend_explicit = backend is not None and backend != BackendType.AUTO
    ...
    def _initialize_weights(self):
        ...
        if self.backend == BackendType.TORCH:
            ...
            self.weight = torch.randn(
                ...
                requires_grad=True)
            if self.bias_flag:
                bias_tensor = torch.zeros(self.out_channels, requires_grad=True)
                # Always expose bias as tensor for tests
                self.bias_param = bias_tensor
            ...
            if self.bias_flag:
                ...
                self.bias_param = self.tensor_ops.create_tensor(
                    bias_data, requires_grad=True)
                self.bias = self.bias_param
            else:
                self.bias_param = None
                self.bias = None
        ...
        if self.backend == BackendType.TORCH:
            ...
            if getattr(self, 'bias_param', None) is not None:
            ...
            if getattr(self, 'bias_param', None) is not None:
                self.bias_param = self.bias_param * 0.0

    def _apply_fractional_derivative(self, x: Any) -> Any:
    ...
    def forward(self, x: Any, tgt: Any = None) -> Any:
        ...
        if self.config.use_fractional:
            x = self._apply_fractional_derivative(x)

        return self._apply_convolution(x)

    def _apply_convolution(self, x: Any) -> Any:
        """Dispatch to backend-specific convolution (for test patching)."""
        if self.backend == BackendType.TORCH:
...
class FractionalTransformer:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Capture encoder/decoder layer counts if provided; default to 6 like torch
        ...
        self.nhead = n_heads
        # Allow override via dim_feedforward kwarg
        if 'dim_feedforward' in kwargs and kwargs['dim_feedforward'] is not None:
            self.d_ff = int(kwargs['dim_feedforward'])
        else:
            self.d_ff = d_ff
        # Expose alias for tests
        self.dim_feedforward = self.d_ff
        self.dropout = dropout
...
class FractionalPooling:
    ...
    def __init__(
        ...
        self.config = config or LayerConfig()
        # Store original values for 1D tests, convert to 2D for comprehensive tests
        self.kernel_size = kernel_size
        self.stride = stride if stride is not None else kernel_size
        self.padding = padding
        
        # For 2D operations, convert to tuples (but keep original format for main attributes)
        if isinstance(kernel_size, int):
            self.kernel_size_2d = (kernel_size, kernel_size)
        else:
            self.kernel_size_2d = kernel_size
        if stride is None:
            self.stride_2d = self.kernel_size_2d
        else:
            self.stride_2d = (stride, stride) if isinstance(stride, int) else stride
        self.padding_2d = (padding, padding) if isinstance(padding, int) else padding
        # Dimension hint for tests (allow explicit override)
        if dim is not None:
            self.dim = int(dim)
        else:
            self.dim = 1 if isinstance(kernel_size, int) else 2
        # Support alias name expected by tests
        self.pooling_type = pool_type if pool_type is not None else pooling_type
        # Also expose the alias attribute for assertions
        self.pool_type = self.pooling_type
    ...
    def _torch_pooling(self, x: Any) -> Any:
        ...
        # Normalize parameters for 1D or 2D based on input rank
        if getattr(x, 'dim', lambda: len(getattr(x, 'shape', [])))() == 3:
            # N, C, L -> 1D pooling
            ...
            if self.pooling_type == "max":
                ...
                return F.avg_pool1d(x, k, s, p)
        else:
            # Assume 2D
            k2 = self.kernel_size_2d
            s2 = self.stride_2d
            p2 = self.padding_2d
            if self.pooling_type == "max":
                return F.max_pool2d(x, k2, s2, p2)
            elif self.pooling_type == "avg":
                return F.avg_pool2d(x, k2, s2, p2)
...
class FractionalLayerNorm:
    ...
    def __init__(
        ...
        self.config = config or LayerConfig()
        # Accept int or tuple like torch.nn.LayerNorm
        if isinstance(normalized_shape, int):
            if normalized_shape <= 0:
                raise ValueError("normalized_shape must be positive integer")
        elif isinstance(normalized_shape, tuple):
            if len(normalized_shape) == 0 or any(int(d) <= 0 for d in normalized_shape):
                raise ValueError("normalized_shape elements must be positive")
        else:
            raise ValueError("normalized_shape must be int or tuple of ints")
    ...
    def forward(self, x: Any) -> Any:
        ...
        if self.backend == BackendType.TORCH:
            import torch
            # For normalized_shape=int, normalize over the last dimension
            # For normalized_shape=tuple, normalize over the last len(tuple) dimensions
            if isinstance(self.normalized_shape, int):
                dim = -1
                # Check if the last dimension matches normalized_shape
                if x.shape[-1] != self.normalized_shape:
                    # If not, normalize over all dimensions except the first (batch)
                    dim = list(range(1, x.dim()))
            else:
                # For tuple, normalize over the last len(tuple) dimensions
                dim = list(range(-len(self.normalized_shape), 0))
            ...
            if self.elementwise_affine:
                if isinstance(self.normalized_shape, int):
                    if x.shape[-1] == self.normalized_shape:
                        # Standard case: normalize over last dimension
                        shape = [1] * (x.dim() - 1) + [self.normalized_shape]
                        w = self.weight.view(shape)
                        b = self.bias.view(shape)
                    else:
                        # Non-standard case: normalize over all non-batch dims
                        # Broadcast weight and bias to match the normalized dimensions
                        shape = [1] + [1] * (x.dim() - 2) + [self.normalized_shape]
                        w = self.weight.view(shape)
                        b = self.bias.view(shape)
                else:
                    # For tuple, use the shape directly
                    w = self.weight
                    b = self.bias
                x_norm = x_norm * w + b
            return x_norm
