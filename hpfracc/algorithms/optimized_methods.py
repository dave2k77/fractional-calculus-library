...
if JAX_AVAILABLE:
    ...
    def _riemann_liouville_jax(f: jnp.ndarray, alpha: float, n: int, h: float) -> jnp.ndarray:
        ...
        integral_part = jax_convolve(f, b, mode='full')[:N] * h**beta / jax_gamma(beta + 1)
        ...
        result = jax.lax.fori_loop(0, n, grad_body, result)
        return result
    ...
    def _caputo_jax(f: jnp.ndarray, alpha: float, h: float) -> jnp.ndarray:
        ...
        integral = jax_convolve(f_deriv, b, mode='full')[:N] * h**beta / jax_gamma(beta + 1)
        return integral
...
def _riemann_liouville_numpy(f: np.ndarray, alpha: float, n: int, h: float) -> np.ndarray:
    ...
    integral_part = convolve(f, b, mode='full')[:N] * h**beta / gamma_func(beta + 1)
...
class FractionalOperator:
    def __init__(self, order: Union[float, FractionalOrder]):
        ...
        if self.alpha.alpha < 0:
            raise ValueError("Order must be non-negative for fractional derivative")
    ...
    def compute(self, f: Union[Callable, np.ndarray, "jnp.ndarray"], t: Union[float, np.ndarray], h: float = None) -> np.ndarray:
        ...
        if JAX_AVAILABLE:
            ...
            result = self._compute_jit(f_array_jax, self.alpha.alpha, step_size)
    ...
    def _get_t_array(self, f: Callable, t: Union[float, np.ndarray], h: float) -> np.ndarray:
        ...
        if callable(f):
            ...
            if t_is_array:
                ...
                raise ValueError("Time array `t` must be provided for array input `f` if `t` is not an array.")
...
class OptimizedGrunwaldLetnikov(FractionalOperator):
    ...
    """
    def _get_jax_kernel(self):
        return jax.jit(_grunwald_letnikov_jax, static_argnums=(1,))
...
class OptimizedRiemannLiouville(FractionalOperator):
    ...
    def compute(self, f: Union[Callable, np.ndarray, "jnp.ndarray"], t: Union[float, np.ndarray], h: float = None) -> np.ndarray:
        ...
        if JAX_AVAILABLE:
            f_array_jax = jnp.asarray(f_array)
            result = self._compute_jit(f_array_jax, self.alpha.alpha, self.n, step_size)
            return np.asarray(result)
        else:
            return self._numpy_kernel(f_array, self.alpha.alpha, step_size)

    def _get_jax_kernel(self):
        return jax.jit(_riemann_liouville_jax, static_argnums=(2,))
...
class OptimizedCaputo(FractionalOperator):
    ...
    def _get_jax_kernel(self):
        return jax.jit(_caputo_jax, static_argnums=(1,))
