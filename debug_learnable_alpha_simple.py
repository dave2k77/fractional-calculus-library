"""
Debug learnable α issue - simple approach
"""
...
class BoundedAlphaParameter(nn.Parameter):
    """Simple bounded α parameter."""
    
    def __init__(self, alpha_init: float = 0.5, alpha_min: float = 0.01, alpha_max: float = 1.99):
        # Transform to unbounded space
        rho_init = torch.tensor(torch.logit(torch.tensor((alpha_init - alpha_min) / (alpha_max - alpha_min))))
...
def debug_learnable_alpha():
    ...
    print("Debugging Learnable α (Simple)...")
