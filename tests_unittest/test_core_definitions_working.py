"""
Working unittest tests for HPFRACC core definitions
Targeting high coverage for hpfracc/core/definitions.py
"""
...
class TestFractionalOrderWorking(unittest.TestCase):
    """Working tests for FractionalOrder class"""
    ...
    def test_fractional_order_properties(self):
        ...
        # Test is_integer property (not method)
...
class TestDefinitionTypeWorking(unittest.TestCase):
    """Working tests for DefinitionType enum"""
...
class TestCaputoDefinitionWorking(unittest.TestCase):
    """Working tests for CaputoDefinition class"""
    ...
    def test_caputo_definition_initialization(self):
        ...
        self.assertEqual(caputo.alpha.alpha, 0.5)  # Access the FractionalOrder's alpha
        self.assertEqual(caputo.definition_type.value, "caputo")
        
        # Test with FractionalOrder
        order = self.FractionalOrder(0.7)
        caputo = self.CaputoDefinition(order)
        self.assertEqual(caputo.alpha.alpha, 0.7)
...
class TestRiemannLiouvilleDefinitionWorking(unittest.TestCase):
    """Working tests for RiemannLiouvilleDefinition class"""
...
class TestGrunwaldLetnikovDefinitionWorking(unittest.TestCase):
    """Working tests for GrunwaldLetnikovDefinition class"""
...
class TestFractionalIntegralWorking(unittest.TestCase):
    """Working tests for FractionalIntegral class"""
    ...
    def test_fractional_integral_initialization(self):
        ...
        with self.assertRaises(ValueError):
            self.FractionalIntegral(0.0)
        
        # Test with negative (should fail)
        with self.assertRaises(ValueError):
            self.FractionalIntegral(-0.1)
...
class TestFractionalCalculusPropertiesWorking(unittest.TestCase):
    """Working tests for FractionalCalculusProperties class"""
...
class TestUtilityFunctionsWorking(unittest.TestCase):
    ...
    def test_create_definition(self):
        """Test create_definition function"""
        ...
        for def_type in valid_types:
            with self.subTest(definition_type=def_type):
                ...
                self.assertEqual(definition.alpha.alpha, 0.5)
        ...
        self.assertEqual(definition.alpha.alpha, 0.7)
    ...
    def test_validate_fractional_order(self):
        ...
        # Test valid orders (excluding 5.0 which might be invalid)
        valid_orders = [0.0, 0.1, 0.5, 0.9, 1.0, 1.5, 2.0]
        for order_val in valid_orders:
            with self.subTest(order=order_val):
        ...
        # Test with custom range (using correct parameter names)
        self.assertTrue(self.validate_fractional_order(0.5, min_value=0.0, max_value=1.0))
        self.assertFalse(self.validate_fractional_order(1.5, min_value=0.0, max_value=1.0))
        self.assertFalse(self.validate_fractional_order(-0.1, min_value=0.0, max_value=1.0))
...
class TestIntegrationWorking(unittest.TestCase):
    ...
    def test_fractional_order_with_definitions(self):
        ...
        self.assertEqual(caputo.alpha.alpha, 0.5)
        self.assertEqual(rl.alpha.alpha, 0.5)
        self.assertEqual(gl.alpha.alpha, 0.5)
    ...
    def test_comprehensive_edge_cases(self):
        ...
        # Test integral with positive values
