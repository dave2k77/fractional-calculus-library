...
class ScalingAnalysis:
    ...
    def analyze_computational_scaling(self, grid_sizes: List[int] = None) -> Dict[str, Any]:
            ...
            for method_name, method_class in method_classes.items():
                try:
                    ...
                    method_results[method_name] = {
                        ...
                        'complexity_ratio': computation_time / (N * np.log(N)) if N > 1 else 0
                    }
                    
                    print(f"  ✅ {method_name}: {computation_time:.4f}s, "
                          f"{memory_usage:.2f} MB, {operations_per_second:.0f} ops/s")
                    ...
                    method_results[method_name] = {
                        ...
                        'memory': 0,
                        'ops_per_second': 0,
                        'complexity_ratio': np.inf
                    }
            
            results[N] = method_results
        
        self.results['computational_scaling'] = results
        return results
    
    def analyze_parallel_scaling(self, grid_sizes: List[int] = None, 
            ...
            for backend in backends:
                ...
                for n_workers in worker_counts:
                    try:
                        ...
                        parallel_computing = ParallelFractionalComputing(
        ...
        for N in grid_sizes:
            ...
            for method_name, method_class in method_classes.items():
                try:
                    ...
                    memory_efficiency = result_memory / memory_used if memory_used > 0 else 0
                    ...
                    print(f"  ✅ {method_name}: {memory_used:.2f} MB, "
    ...
    def analyze_optimization_scaling(self, grid_sizes: List[int] = None) -> Dict[str, Any]:
        ...
        for N in grid_sizes:
            ...
            try:
                ...
                backend_results['JAX GPU'] = {
                    ...
                    'throughput': N / (end_time - start_time) if (end_time - start_time) > 0 else 0
                }
            ...
            for method_name, method_class in method_classes.items():
                try:
                    ...
                    method_results[method_name] = {
                        ...
                        'accuracy_per_point': -np.log10(mean_error) if mean_error > 0 else np.inf
                    }
                    
                    print(f"  ✅ {method_name}: max error {max_error:.2e}, "
    ...
    def generate_scaling_report(self) -> str:
        ...
        report.append(f"  Available Memory: {self.system_info['memory_available'] / 1024**3:.2f} GB")
                ...
                for method, data in methods.items():
                    if data['time'] < np.inf:
                        report.append(f"    {method}: {data['time']:.4f}s, "
                                   f"{data['ops_per_second']:.0f} ops/s")
            report.append("")
            ...
            for N, backends in self.results['parallel_scaling'].items():
                ...
                for backend, workers in backends.items():
                    for n_workers, data in workers.items():
                        if data['time'] < np.inf:
                            report.append(f"    {backend} ({n_workers} workers): "
            ...
            for N, methods in self.results['accuracy_scaling'].items():
                ...
                for method, data in methods.items():
                    if data['max_error'] < np.inf:
                        report.append(f"    {method}: max error {data['max_error']:.2e}, "
        ...
        print("✅ Scaling analysis report generated and saved to 'benchmarks/scaling_analysis_report.txt'")
        return report_text
    ...
    def plot_scaling_results(self):
        ...
        if 'computational_scaling' in self.results:
            ...
            for method in methods:
                memory_usage = []
                for N in grid_sizes:
                    memory_val = self.results['computational_scaling'][N][method]['memory']
            ...
            for method in methods:
                throughput = []
                for N in grid_sizes:
                    ops_val = self.results['computational_scaling'][N][method]['ops_per_second']
                    throughput.append(ops_val if ops_val > 0 else np.nan)
                
                plt.loglog(grid_sizes, throughput, 'o-', label=method, linewidth=2, markersize=8)
            ...
            for method in methods:
                complexity = []
                for N in grid_sizes:
                    comp_val = self.results['computational_scaling'][N][method]['complexity_ratio']
                    complexity.append(comp_val if comp_val < np.inf else np.nan)
                
                plt.loglog(grid_sizes, complexity, 'o-', label=method, linewidth=2, markersize=8)
            ...
            plt.tight_layout()
            plt.savefig('benchmarks/plots/computational_scaling.png', dpi=300, bbox_inches='tight')
            plt.show()
        ...
        if 'parallel_scaling' in self.results:
            ...
            worker_counts = list(self.results['parallel_scaling'][grid_sizes[0]][backends[0]].keys())
            ...
            for i, backend in enumerate(backends):
                ...
                for N in grid_sizes:
                    times = []
                    for n_workers in worker_counts:
                        time_val = self.results['parallel_scaling'][N][backend][n_workers]['time']
                        times.append(time_val if time_val < np.inf else np.nan)
                    
                    plt.semilogy(worker_counts, times, 'o-', label=f'N={N}', linewidth=2, markersize=8)
            ...
            plt.tight_layout()
            plt.savefig('benchmarks/plots/parallel_scaling.png', dpi=300, bbox_inches='tight')
            plt.show()
        ...
        if 'memory_scaling' in self.results:
            ...
            for method in methods:
                memory_usage = []
                for N in grid_sizes:
                    memory_val = self.results['memory_scaling'][N][method]['memory_used']
                    memory_usage.append(memory_val)
            ...
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig('benchmarks/plots/memory_scaling.png', dpi=300, bbox_inches='tight')
            plt.show()
        ...
        if 'accuracy_scaling' in self.results:
            ...
            for method in methods:
                ...
                plt.loglog(grid_sizes, errors, 'o-', label=method, linewidth=2, markersize=8)
            ...
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig('benchmarks/plots/accuracy_scaling.png', dpi=300, bbox_inches='tight')
            plt.show()
        
        print("✅ Scaling analysis plots generated and saved to 'benchmarks/plots/' directory")
