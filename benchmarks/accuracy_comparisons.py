...
class AccuracyBenchmark:
    ...
    def _define_analytical_solutions(self) -> Dict[str, Dict[str, Callable]]:
        ...
        caputo_solutions = {
            ...
            'power': lambda t, alpha: gamma(1.5) * t**(0.5-alpha) / gamma(1.5-alpha),
    ...
    def benchmark_accuracy(self, grid_sizes: List[int] = None, 
                ...
                for N in grid_sizes:
                    ...
                    for method_name, method_class in method_classes.items():
                        try:
                            ...
                            if method_name in ['Caputo', 'FFT Spectral', 'FFT Convolution']:
                                analytical = self.analytical_solutions['caputo'][func_name](t, alpha)
                            elif method_name == 'Riemann-Liouville':
                                analytical = self.analytical_solutions['riemann_liouville'][func_name](t, alpha)
                            else:
                    ...
                    best_method = min(method_results.items(), 
                                    key=lambda x: x[1]['max_error'] if x[1]['max_error'] < np.inf else np.inf)
                    print(f"    Grid {N}: Best method {best_method[0]} "
                          f"(max error: {best_method[1]['max_error']:.2e})")
        
        self.results['accuracy'] = results
        return results
    
    def benchmark_convergence_rates(self, grid_sizes: List[int] = None) -> Dict[str, Any]:
    ...
    def benchmark_stability_analysis(self, grid_sizes: List[int] = None) -> Dict[str, Any]:
        ...
        for func_name, func in test_functions.items():
            ...
            for alpha in alpha_values:
                ...
                for N in grid_sizes:
                    ...
                    for method_name, method_class in method_classes.items():
                        try:
                            ...
                            method_results[method_name] = {
                                ...
                                'has_nan': has_nan,
                                'has_inf': has_inf,
                                'is_stable': not (has_nan or has_inf or condition_number > 1e10)
                            }
                    ...
                    stable_methods = [name for name, data in method_results.items() if data['is_stable']]
    ...
    def generate_accuracy_report(self) -> str:
            ...
            for alpha, functions in self.results['accuracy'].items():
                ...
                for func_name, grid_sizes in functions.items():
                    ...
                    for N, methods in grid_sizes.items():
                        best_method = min(methods.items(), 
                                        key=lambda x: x[1]['max_error'] if x[1]['max_error'] < np.inf else np.inf)
                        report.append(f"      Grid {N}: {best_method[0]} "
                                   f"(max error: {best_method[1]['max_error']:.2e})")
            report.append("")
        ...
        if 'stability' in self.results:
            ...
            for func_name, alphas in self.results['stability'].items():
                ...
                for alpha, grid_sizes in alphas.items():
                    for N, methods in grid_sizes.items():
                        stable_methods = [name for name, data in methods.items() if data['is_stable']]
                        report.append(f"    α={alpha}, Grid {N}: Stable methods: {stable_methods}")
            report.append("")
        ...
        print("✅ Accuracy report generated and saved to 'benchmarks/accuracy_report.txt'")
        return report_text
    ...
    def plot_accuracy_results(self):
        ...
        if 'accuracy' in self.results:
            ...
            if alpha in self.results['accuracy'] and func_name in self.results['accuracy'][alpha]:
                grid_sizes = list(self.results['accuracy'][alpha][func_name].keys())
                methods = list(self.results['accuracy'][alpha][func_name][grid_sizes[0]].keys())
                
                for method in methods:
                    errors = []
                    for N in grid_sizes:
                        error = self.results['accuracy'][alpha][func_name][N][method]['max_error']
                ...
                plt.tight_layout()
                plt.savefig('benchmarks/plots/accuracy_comparison.png', dpi=300, bbox_inches='tight')
                plt.show()
            ...
            if func_name in self.results['convergence']:
                ...
                for method in methods:
                    ...
                    if not np.isnan(data['convergence_rate']):
                        ...
                        errors = data['errors']
                        valid_data = [(h, e) for h, e in zip(h_vals, errors) if e < np.inf]
                        
                        if valid_data:
                            h_vals, err_vals = zip(*valid_data)
                            plt.loglog(h_vals, err_vals, 'o-', label=f'{method} (rate: {data["convergence_rate"]:.2f})', 
                ...
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
                plt.savefig('benchmarks/plots/convergence_analysis.png', dpi=300, bbox_inches='tight')
                plt.show()
        ...
        if 'stability' in self.results:
            ...
            if func_name in self.results['stability'] and alpha in self.results['stability'][func_name]:
                grid_sizes = list(self.results['stability'][func_name][alpha].keys())
                methods = list(self.results['stability'][func_name][alpha][grid_sizes[0]].keys())
                
                for method in methods:
                    condition_numbers = []
                    for N in grid_sizes:
                        cn = self.results['stability'][func_name][alpha][N][method]['condition_number']
                        condition_numbers.append(cn if cn < np.inf else np.nan)
                    
                    plt.semilogy(grid_sizes, condition_numbers, 'o-', label=method, linewidth=2, markersize=8)
                ...
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
                plt.savefig('benchmarks/plots/stability_analysis.png', dpi=300, bbox_inches='tight')
                plt.show()
