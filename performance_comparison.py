...
class PerformanceBenchmark:
    ...
    def benchmark_method(self, method_name: str, method_func: Callable, 
                        test_params: Dict, n_runs: int = 5) -> Dict:
    ...
    def run_comparison(self, alpha: float = 0.5, sizes: List[int] = None) -> Dict:
        ...
        for size in sizes:
            ...
            # Current methods
            current_methods = {
                'Caputo (Direct)': lambda **params: CaputoDerivative(alpha, method="direct").compute(**params),
                'Caputo (L1)': lambda **params: CaputoDerivative(alpha, method="l1").compute(**params),
                'RL (Direct)': lambda **params: RiemannLiouvilleDerivative(alpha, method="direct").compute(**params),
                'RL (FFT)': lambda **params: RiemannLiouvilleDerivative(alpha, method="fft").compute(**params),
                'GL (Direct)': lambda **params: GrunwaldLetnikovDerivative(alpha, method="direct").compute(**params),
            }
            
            # Optimized methods
            optimized_methods = {
                'Caputo (L1 Optimized)': lambda **params: OptimizedCaputo(alpha).compute(**params, method="l1"),
                'Caputo (Diethelm-Ford-Freed)': lambda **params: OptimizedCaputo(alpha).compute(**params, method="diethelm_ford_freed"),
                'RL (FFT Optimized)': lambda **params: OptimizedRiemannLiouville(alpha).compute(**params),
                'GL (JAX Optimized)': lambda **params: OptimizedGrunwaldLetnikov(alpha).compute(**params),
            }
            ...
            for category in ['current', 'optimized']:
                for method_name, result in results[size][category].items():
                    ...
                    if method_name in results[size][category]:
                        avg_times.append(results[size][category][method_name]['avg_time'])
                        std_times.append(results[size][category][method_name]['std_time'])
                    else:
        ...
        for size in sizes:
            for method in current_methods:
                if method in results[size]['current']:
                    ...
                    optimized_name = method.replace(' (Direct)', ' (L1 Optimized)').replace(' (FFT)', ' (FFT Optimized)').replace(' (L1)', ' (L1 Optimized)')
                    
                    if optimized_name in results[size]['optimized']:
                        optimized_time = results[size]['optimized'][optimized_name]['avg_time']
    ...
    def print_summary(self, results: Dict):
        ...
        for size in results.keys():
            ...
            for method_name, result in results[size]['current'].items():
                print(f"  {method_name:30s}: {result['avg_time']:8.4f}s ± {result['std_time']:6.4f}s")
